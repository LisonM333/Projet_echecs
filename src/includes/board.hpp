...
class Board {
    ...
    std::pair<Piece*, Piece*> get_kings();
    ...
    char get_char(const int& line,const int& colum) const;
    ...
    bool square_is_empty(const Position& square) const {if (square.x != 8){return (m_lines[square.x][square.y]== nullptr);} else {return false;}};
     ...
     // T and Q are different only for the case of the comparaison between char* and string
    ...
    void move_verifying(const Position& start, const Position& end);
};
